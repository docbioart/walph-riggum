...
> Generated by Good Bunny on 2026-02-10. Run `goodbunny fix` to address findings.
...
- [ ] **[SECURITY]** `--dangerously-skip-permissions` used in Jeeroy analysis without necessity: In `jeeroy.sh`, the `run_analysis()` function (line ~346) and `run_direct_generation()` function (line ~440) call Claude with `claude -p` (print mode) but do NOT use `--dangerously-skip-permissions`. However, the `run_qa_session()` function (line ~400-403) uses `--dangerously-skip-permissions` for the interactive Q&A session where Claude writes spec files to disk. While this is intentional for file writing, the Q&A session also has full shell access, and users may not realize the security implications since the Q&A mode is described as "interactive" and "clarifying questions." The risk is that a malicious document could cause Claude to execute arbitrary commands during the Q&A session.
  - File(s): `jeeroy.sh` (lines ~400-403)
  - Fix: Add a clear warning to the user before launching the Q&A session with `--dangerously-skip-permissions`, and document this in the help text. Consider using a more restricted permission model for the Q&A session (e.g., only allowing file writes to the `specs/` directory) if Claude CLI supports it.
...
- [ ] **[SECURITY]** `source "$project_config"` executes arbitrary code from user-controlled config files: Both `lib/config.sh:load_config()` (line ~37) and `goodbunny.sh:load_goodbunny_config()` (line ~142) use `source` to load config files from the project directory. Since `.walph/config` and `.goodbunny/config` are project files that could be committed to a repository, a malicious repository could include config files containing arbitrary shell commands that execute when `walph` or `goodbunny` is run.
  ...
  - Fix: Instead of `source`-ing the config files, parse them as key-value pairs using a safe reader. For example, read each line, validate it matches `^[A-Z_]+=.*$`, and then use `eval` only on validated lines, or use `grep` + `sed` to extract known keys. Alternatively, add a comment warning users that config files are `source`-d and should only come from trusted sources.

## High Priority

- [ ] **[ERROR HANDLING]** `check_api_error` has overly broad pattern matching that will cause false positives: In `lib/status_parser.sh` (line ~152), the function `check_api_error()` greps for `500|503|overloaded` in the raw Claude output. Since Claude output can contain arbitrary text (including code examples, documentation, or log output that mention "500" or "503"), this will trigger false API error detection on benign output containing these numbers.
  - File(s): `lib/status_parser.sh` (lines ~149-156)
  - Fix: Make the pattern more specific. Grep for patterns that look like actual HTTP errors, e.g., `"Error: 500"`, `"HTTP 503"`, `"status.*500"`, or look for specific Claude CLI error message formats rather than bare numbers.
...
- [ ] **[ERROR HANDLING]** `parse_args` in `walph.sh` doesn't validate `--max-iterations` or `--model` arguments: At lines ~84-89 in `walph.sh`, `--max-iterations` does `shift 2` without checking if `$2` exists or is a number. Running `walph build --max-iterations` (without a value) will cause an unbound variable error or silently use the next argument as the number. Same issue with `--model` at line ~88.
  - File(s): `walph.sh` (lines ~84-89)
  - Fix: Add validation before `shift 2`: check that `$#` is >= 2, and for `--max-iterations`, validate that `$2` is a positive integer. Show a helpful error message if validation fails.
...
- [ ] **[ERROR HANDLING]** Temp files in `walph.sh` and `goodbunny.sh` `run_iteration()` may leak on unexpected exit: In both `walph.sh` (lines ~1362-1367) and `goodbunny.sh` (lines ~357-362), temp files are created with `mktemp` but are only cleaned up in the happy path with `rm -f`. If the script is interrupted (SIGINT/SIGTERM) between temp file creation and cleanup, the temp files will remain on disk. Unlike `jeeroy.sh` which has a proper `trap cleanup_temp_files EXIT INT TERM` pattern, the other scripts don't trap for temp file cleanup.
...
- [ ] **[COMPLEXITY]** `_relative_path()` in `converter.sh` shells out to Python as a fallback: In `lib/converter.sh` (lines ~162-178), the `_relative_path()` function tries `realpath --relative-to`, then falls back to `python3 -c "import os; print(os.path.relpath(...))"`, then to `basename`. The Python fallback is a heavyweight solution for a simple path operation and introduces a dependency on Python being installed.

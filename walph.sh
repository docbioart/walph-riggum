...
run_init() {
    ...
    # Specs may already exist (e.g. generated by Jeeroy) — keep them and
    # tailor the next-steps message accordingly
    local existing_specs
    existing_specs=$(count_spec_files "$target_dir/specs")
    if [[ "$existing_specs" -gt 0 ]]; then
        log_info "Found $existing_specs existing spec file(s) in specs/ - keeping them"
    fi
...
EOF
    ...
    if [[ "$existing_specs" -gt 0 ]]; then
        ...
        echo "  2. Review the $existing_specs existing spec(s) in specs/"
...
main() {
    ...
    if [[ ! -d "$PROJECT_DIR/.walph" ]] && [[ ! -f "$PROJECT_DIR/AGENTS.md" ]]; then
        # Jeeroy handoff: specs already generated, but 'walph setup' never ran
        local spec_count
        spec_count=$(count_spec_files "$PROJECT_DIR/specs")
        if [[ "$spec_count" -gt 0 ]]; then
            log_info "Found $spec_count spec file(s) in specs/ but no .walph configuration."
            log_info "Looks like specs were already generated (e.g. by Jeeroy) — setup just hasn't run yet."
            if [[ ! -t 0 ]] || ask_yes_no "Run 'walph setup' now?" "y"; then
                WALPH_SETUP_INLINE=true run_setup
            else
                log_info "Run 'walph setup' first, then re-run 'walph $MODE'."
                exit 0
            fi
        else

...
└── IMPLEMENTATION_PLAN.md  # 3. Task list (generated by ralph plan)
```
...
```bash
../ralph.sh plan --max-iterations 2
```
...
```bash
../ralph.sh build --max-iterations 20
```
...
./ralph.sh plan [--max-iterations N] [--model opus]

# Run building (implements from plan)
./ralph.sh build [--max-iterations N] [--model sonnet]
...
6. **Watch the logs** - `tail -f .ralph/logs/*.log`

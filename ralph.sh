...
show_init_help() {
    cat << 'EOF'
Ralph Wiggum - Initialize Project

USAGE:
    ralph.sh init [project-name] [options]
...
EXAMPLES:
    ralph.sh init my-api --template api --stack node
    ralph.sh init my-app --template fullstack
    ralph.sh init my-ios-app --template ios
    ralph.sh init my-mobile --template capacitor
    ralph.sh init my-tool --template cli --stack python
...
WHAT IT CREATES:
    ...
    IMPLEMENTATION_PLAN.md  # Task list (generated by 'ralph plan')
EOF
}

show_templates() {
    cat << 'EOF'
Ralph Wiggum - Architecture Templates
...
Use: ralph.sh init my-app --template <name>
EOF
}
...
show_status() {
    ...
    if [[ -d "$PROJECT_DIR/.ralph" ]]; then
        ...
        if [[ -f "$PROJECT_DIR/.ralph/state/circuit_breaker.json" ]]; then
            init_circuit_breaker "$PROJECT_DIR/.ralph/state"
        ...
        if [[ -f "$PROJECT_DIR/IMPLEMENTATION_PLAN.md" ]]; then
            ...
            echo "Implementation plan: Not found (run 'ralph plan' first)"
        fi
    else
        echo "Ralph initialized: No (run 'ralph init' in project directory)"
    fi
}
...
reset_state() {
    ...
    if [[ -d "$PROJECT_DIR/.ralph/state" ]]; then
        rm -f "$PROJECT_DIR/.ralph/state/"*.json
...
run_init() {
    ...
    log_info "Creating .ralph directory..."
    mkdir -p "$target_dir/.ralph/logs"
    mkdir -p "$target_dir/.ralph/state"

    # Copy prompt templates
    if [[ -f "$SCRIPT_DIR/templates/PROMPT_plan.md" ]]; then
        cp "$SCRIPT_DIR/templates/PROMPT_plan.md" "$target_dir/.ralph/"
    fi
    if [[ -f "$SCRIPT_DIR/templates/PROMPT_build.md" ]]; then
        cp "$SCRIPT_DIR/templates/PROMPT_build.md" "$target_dir/.ralph/"
    fi

    # Create config file
    cat > "$target_dir/.ralph/config" << 'EOF'
...
3. Run `ralph plan` to generate tasks
4. Run `ralph build` to implement
...
> This file is generated by `ralph plan`. Write your specs in `specs/` first.

## Overview

<!-- Will be filled by ralph plan -->
...
Run `ralph plan` to generate tasks from your specs.
EOF
...
EOF
    ...
    if [[ ! -d "$target_dir/.git" ]]; then
        ...
        (cd "$target_dir" && git init && git add . && git commit -m "Initialize Ralph Wiggum project") 2>/dev/null || true
    fi
    ...
    echo "  4. Run: ralph plan"
    echo "  5. Review IMPLEMENTATION_PLAN.md"
    echo "  6. Run: ralph build"
}
...
main_loop() {
    ...
    while [[ $iteration -le $MAX_ITERATIONS ]]; do
        ...
        if circuit_breaker_triggered; then
            ...
            log_info "Run 'ralph reset' to clear the circuit breaker"
            return 1
        fi

        # Select prompt and model
        local prompt_file
        if [[ -f "$PROJECT_DIR/.ralph/PROMPT_${MODE}.md" ]]; then
            prompt_file="$PROJECT_DIR/.ralph/PROMPT_${MODE}.md"
        ...
        if [[ -f "$PROJECT_DIR/.ralph/state/completion_signal" ]]; then
            log_success "All tasks completed!"
            rm -f "$PROJECT_DIR/.ralph/state/completion_signal"
            return 0
        fi
...
main() {
    ...
    if [[ ! -d "$PROJECT_DIR/.ralph" ]] && [[ ! -f "$PROJECT_DIR/AGENTS.md" ]]; then
        log_warn "This doesn't appear to be a Ralph-enabled project."
        log_info "Run 'ralph init' or create a .ralph directory first."
